...
class _MainLayoutState extends State<MainLayout> {
  ...
  // Pořadí odpovídá spodní navigační liště níže. Věž dostává vlastní tematické pozadí (studená
  // modro-šedá "starobylá věž") - na rozdíl od Lairu/Arény/Riftu není otevíraná přes
  // openWorldScreen (je to permanentní tab), takže gradient je potřeba obalit přímo tady.
  static final _navScreens = [
    Container(
      decoration: const BoxDecoration(gradient: RadialGradient(center: Alignment.topCenter, radius: 1.4, colors: [Color(0x291E88E5), FantasyColors.abyss])),
      child: const TowerScreen(),
    ),
    const HubScreen(),
    Container(
        ...
        actions: [
          ElevatedButton(
            ...
            onPressed: () {
              ...
              state.confirmDeath();
              // confirmDeath() vynuluje heroClass na none a výběr povolání se zobrazuje jen na
              // Věž Osudu (index 0) - bez tohohle by hráč po smrti zůstal třeba v Batohu/Profilu
              // a nikde by neviděl, že si má vybrat nové povolání.
  ...
  Widget _navSlot(FantasyIconType iconType, String label, int index, {GameState? state}) {
    ...
    final bool showNewBadge = index == 1 && state != null && state.hasUnseenWorldUnlock;
    return InkWell(
      onTap: () => setState(() {
        _navIndex = index;
        if (index == 1 && state != null) state.markWorldTabSeen();
      }),
        ...
        bottomNavigationBar: Container(
          ...
          child: SafeArea(
            ...
            child: Row(
              ...
              children: [
                _navSlot(FantasyIconType.statSword, tr('Věž Osudu', 'Tower of Fate'), 0),
                _navSlot(FantasyIconType.systemGuild, tr('Svět', 'World'), 1, state: state),
                          ...
                          if (state.portraitCombatMode) ...[
                            Center(
                              child: Column(children: [
                                ...
                                if (state.specialization != 0) Text(state.currentSpecRelic.form, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 10, color: heroAccent.withOpacity(.75))),
                              ]),
                            ),
                            const SizedBox(height: 8),
                          ] else
    ...
    return Consumer<GameState>(builder: (context, state, _) {
      ...
      return ListView(
        ...
        children: [
          ...
          if (!showPotions) ...[
          Builder(builder: (context) {
            final equippedItems = state.inventory.where((i) => i.isActive).toList();
            if (equippedItems.isEmpty) return const SizedBox.shrink();
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(tr("NASAZENÉ VYBAVENÍ (mimo batoh)", "EQUIPPED GEAR (outside the bag)"), style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 1)),
                const SizedBox(height: 6),
                SizedBox(
                  height: 56,
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, i) {
                      final item = equippedItems[i];
                      return GestureDetector(
                        onTap: () => _showItemDetailDialog(context, state, item),
                        onLongPress: () => _showItemDetailDialog(context, state, item),
                        child: Container(
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(8),
                            border: Border.all(color: item.rarityColor, width: 1.2),
                            boxShadow: rarityCardGlow(item.rarityVisual),
                          ),
                          child: FantasyIconFrame(type: item.iconType, rarity: item.rarityVisual, size: 44, interactive: false),
                        ),
                      );
                    },
                  ),
